...
-- 			TCore_Debug_show_var('line',line )
			if string.sub(line,1,2)=='--' then -- its a comment
				sThirdChar = string.sub(line,3,3)
				if '!' == sThirdChar then -- it's a magic comment
					local magic = string.sub(line,4)
					outStream:writeln('//!' .. magic)
					fn_magic = checkComment4fn(fn_magic,magic)
				elseif '-' == sThirdChar then
					local magic = string.sub(line,4)
					outStream:writeln('/// \\brief ' .. magic)
					bInMagicBlock = true
					local comment = ''
					local thisComment = ''
					while (not err) and (bInMagicBlock) and (not inStream:eof()) do
						line = string_trim(inStream:getLine())
						thisComment = line:match('^--%s(%S.*)')
						if thisComment then
							local sNS = thisComment:match('^namespace:%s(.*)$')
							if sNS then
								local tParts = string_split(sNS, '::')
								for _, sV in ipairs(tParts) do
									sNameSpace = sNameSpace .. 'namespace '
												 .. sV .. ' { '
									sNameSpaceEnd = sNameSpaceEnd .. '} '
								end
								comment = comment .. sNameSpace .. '\n'
							else
						...
						elseif '--' == line then
							...
							bInMagicBlock = false
							inStream:ungetLine(line)
							if 0 < #sNameSpace then

							end
						end
					end -- while loop looking for end of block
			...
			elseif line:match('^(%S+)%s*=%s*%S+') then
				...
				local sVar, sVal = line:match('^(%S+)%s*=%s*(%S+)')
				local sFC = sVal:sub(1, 1)
				local sType = 'auto'
				if ('"' == sFC) or ("'" == sFC) then
					sType = 'string'
				elseif '{' == sFC then
					sType = 'table'
					sVal = '{}'
				elseif tonumber(sFC) then
					sType = 'number'
				end
				sVar = sVar:gsub('%.', '_')
				sVal = sVal:gsub("'", '"')
				outStream:writeln(sType .. ' ' .. sVar .. '=' .. sVal .. ';')
			else
		...
		outStream:writeln(sNameSpaceEnd)
